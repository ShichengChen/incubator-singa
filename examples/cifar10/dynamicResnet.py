...
def dynamicBlock(net, name, nb_filters, stride):
    split = net.add(layer.Split(name + "-split", 2))
    if stride > 1:
        net.add(layer.Conv2D(name + "-br2-conv", nb_filters, 1, stride, pad=0), split)
        br2bn = net.add(layer.BatchNormalization(name + "-br2-bn"))
    net.add(layer.DynamicConv2D(name + "-br1-conv1", nb_filters, 3, stride, pad=1), split)
    net.add(layer.BatchNormalization(name + "-br1-bn1"))
    net.add(layer.Activation(name + "-br1-relu"))
    net.add(layer.DynamicConv2D(name + "-br1-conv2", nb_filters, 3, 1, pad=1))
    br1bn2 = net.add(layer.BatchNormalization(name + "-br1-bn2"))
    if stride > 1:
        net.add(layer.Merge(name + "-merge"), [br1bn2, br2bn])
    else:
...
def create_net(use_cpu=False):
    ...
    Block(net, "2a", 16, 1)
    dynamicBlock(net, "2b", 16, 1)
    dynamicBlock(net, "2c", 16, 1)

    Block(net, "3a", 32, 2)
    dynamicBlock(net, "3b", 32, 1)
    dynamicBlock(net, "3c", 32, 1)

    Block(net, "4a", 64, 2)
    dynamicBlock(net, "4b", 64, 1)
    dynamicBlock(net, "4c", 64, 1)
