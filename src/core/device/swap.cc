...
struct sort_by_ptr_idx_ascending {
  ...
  inline bool operator()(const InfoBlock& struct1, const InfoBlock& struct2) {
    return ((struct1.ptr < struct2.ptr) ||
        ((struct1.ptr == struct2.ptr) && (struct1.idx < struct2.idx)));
  }
};
...
LL SwapGPU::SwapInTime(size_t size) { return (swapincof * size + swapinbias); }
...
vector<SwapBlock> SwapGPU::SelectBlock(vector<SwapBlock>& vec_swap,
                                       vector<LL> temp_load, LL mem_limit) {
  // select the swap blocks
  // you should write the index of selected blocks in rfvariables and then read
  // from there.
  vector<SwapBlock> vec_swap_selct;
  vector<int> checkfile;
  int maxn = 0;
  for (int i = 0; i < (int)vec_swap.size(); i++) checkfile.push_back(1);
  sort(vec_swap.begin(), vec_swap.end(), sort_by_idx_ascending_swap());


  for (int i = 0; i < (int)vec_swap.size() && i < number_of_swap_blocks;
       i++) {
    if (i > maxn)
...
void SwapGPU::StickToLimit(vector<SwapBlock>& vec_swap_selct,
                           vector<LL>& vec_load_temp, LL& overhead,
                           LL mem_limit, string mode) {
  // set the memory limitation that you should not exceed.
  if (mode == "stick-to-limit") {
    overhead = 0;
    sort(vec_swap_selct.begin(), vec_swap_selct.end(),
    ...
    for (int i = 1; i < (int)vec_run.size() && i < max_idx; i++) {
      ...
      while (vec_run[i].t < itm.t_out_end) {
        if (accSmoothLoad[i + 1] - extra0 < mem_limit &&
            accSmoothLoad[i + 2] - extra0 < mem_limit &&
    ...
    sort(vec_swap_selct.begin(), vec_swap_selct.end(),
         sort_by_didx_descending_swap());
    {
      int i;
      for (i = iterlen * 3 - 2, cnt = 0; cnt < (int)vec_swap_selct.size();
           i--) {
        ...
        while (vec_run[i].t > itm.t_in_start) {
          if (accSmoothLoad[i - 1] - extra1 < mem_limit &&
              accSmoothLoad[i - 2] - extra1 < mem_limit &&
...
void SwapGPU::BuildMetaTables(vector<SwapBlock>& vec_swap_selct) {
  ...
  assert(cudaStreamCreateWithFlags(&stream1, cudaStreamNonBlocking) ==
      cudaSuccess);
  assert(cudaStreamCreateWithFlags(&stream2, cudaStreamNonBlocking) ==
      cudaSuccess);
  sort(vec_swap_selct.begin(), vec_swap_selct.end(),
       sort_by_idx_ascending_swap());
  ...
  // make schedule for remove directly blocks, some variables do not need to be
  // swap
...
void SwapGPU::Plan() {
  ...
  */
  ...
  vector<InfoBlock> one_itr(&vec_opt_info[fastinterval],
                            &vec_opt_info[fastinterval + iterlen]);
  ...
  vector<InfoBlock> temp_vec_run(&vec_opt_info[fastinterval],
                                 &vec_opt_info[fastinterval + iterlen * 3]);
  vec_run = temp_vec_run;

  vector<LL> vec_load(&accload[fastinterval],
                      &accload[fastinterval + 3 * iterlen]);
  ...
  // if the second usage of the blocks are free or write, I will remove the
  // blocks directly.
  // when it will be used again, i will malloc space for it.
  vector<SwapBlock> vec_swap;
  for (int i = 1; i < (int)vec_run_dup.size(); i++) {
    if ((vec_run_dup[i].size > (1 << remove_limit)) &&
        (vec_run_dup[i - 1].idx + delay < vec_run_dup[i].idx) &&
        (vec_run_dup[i - 1].ptr == vec_run_dup[i].ptr) &&
        ((vec_run_dup[i - 1].operation_type == READ) ||
            (vec_run_dup[i - 1].operation_type == WRITE) ||
            (vec_run_dup[i - 1].operation_type == MALLOC)) &&
        ((vec_run_dup[i].operation_type == FREE) ||
            (vec_run_dup[i].operation_type == WRITE)) &&
        recomputeUsedBlocks.find(vec_run_dup[i - 1].idx % iterlen) ==
            recomputeUsedBlocks.end()) {
      SwapBlock itm(vec_run_dup[i].ptr, vec_run_dup[i].size,
                    vec_run_dup[i - 1].idx, vec_run_dup[i].idx,
                    vec_run_dup[i - 1].t, vec_run_dup[i].t);
  ...
  for (int i = 0; i < (int)vec_run.size(); i++) {
    LL extra = 0;
    if (openremovedirect && findSmoothL.find(i) != findSmoothL.end())
      extra -= (LL)removeBlock[findSmoothL[i]].size;
    if (openremovedirect && findSmoothR.find(i) != findSmoothR.end())
      extra += (LL)removeBlock[findSmoothR[i]].size;
    if (vec_run[i].operation_type == MALLOC)
      extra += vec_run[i].size;
    else if (vec_run[i].operation_type == FREE)
      extra -= vec_run[i].size;
    if (i == 0)
      accSmoothLoad.push_back(beginv + extra);
    else
      accSmoothLoad.push_back(accSmoothLoad[accSmoothLoad.size() - 1] + extra);
  }
  ...
  for (auto i : vec_run_dup)
    if (i.idx <= max_idx + midremove && i.idx >= max_idx - midremove)
      removed[i.ptr] = 1;
  ...
  for (int i = 1; i < (int)vec_run_dup.size(); i++) {
    if ((vec_run_dup[i].size >= smallest_block) &&
        ...
        (vec_run_dup[i].idx > max_idx + midremove) &&
        (removed.find(vec_run_dup[i].ptr) == removed.end()) &&
        (vec_run_dup[i - 1].ptr == vec_run_dup[i].ptr) &&
        ((vec_run_dup[i - 1].operation_type == READ) ||
            (vec_run_dup[i - 1].operation_type == WRITE) ||
            (vec_run_dup[i - 1].operation_type == MALLOC)) &&
...
void SwapGPU::DetectionIteration() {
  ...
  */
  if (((global_index + 1) % (iterlen_threshold) == 0) &&
      (async_swap_flag == 0) && (past_test_flag == 0)) {
    for (int i = iterlen_threshold; i < (int)vecBlock.size(); i++) {
      if (iterlen > iterlen_threshold) break;
      for (int len = iterlen_threshold; 2 * len + i < (int)vecBlock.size();
           len++) {
        if (iterlen > iterlen_threshold) break;
        if (equal(vecBlock.begin() + i, vecBlock.begin() + i + len,
                  vecBlock.begin() + i + len)) {
          iterlen = len;
          iter2 = i;
        }
      }
    ...
    if (iterlen < iterlen_threshold) {
      ...
      global_index_threshold =
          global_index + iterlen - (global_index - iter2) % iterlen;
      past_test_flag = 1;
...
void SwapGPU::SwapOut(const int idx) {
  ...
  if (meta.out == false && meta.synout == false)
    meta.out = meta.synout = true;
  else
    return;
  LL now0 = std::chrono::duration_cast<std::chrono::nanoseconds>(
      std::chrono::system_clock::now().time_since_epoch())
      .count();
  cudaError_t err =
      cudaMemcpyAsync(meta.cpu_ptr, meta.block_->get_data(), meta.size,
                      cudaMemcpyDeviceToHost, meta.out_stream);
  assert(err == cudaSuccess);
  if (syncfactor) cudaEventRecord(meta.out_event, meta.out_stream);
  table_meta[idx] = meta;
  LL now1 = std::chrono::duration_cast<std::chrono::nanoseconds>(
      std::chrono::system_clock::now().time_since_epoch())
      .count();
...
void SwapGPU::SwapOutSyn(const int idx) {
  ...
  if (meta.out == true && meta.synout == true)
    meta.synout = false;
  else
    return;
  LL now0 = std::chrono::duration_cast<std::chrono::nanoseconds>(
...
void SwapGPU::SwapIn(const int idx) {
  // swap in a variable
  BlockMeta meta = table_meta[idx];
  if (meta.out == true && meta.synout == false && meta.synin == false)
    meta.synin = true;
  else
    return;
  LL now0 = std::chrono::duration_cast<std::chrono::nanoseconds>(
      ...
      .count();
  swapload[swapload.size() - 1] += (LL)meta.size;
  assert(cudaMemcpyAsync(meta.block_->get_data(), meta.cpu_ptr, meta.size,
                         cudaMemcpyHostToDevice,
                         meta.in_stream) == cudaSuccess);
...
void SwapGPU::SwapInSyn(const int idx) {
  ...
  auto meta = table_meta[idx];
  if (meta.out == true && meta.synout == false && meta.synin == true)
    meta.synin = meta.out = false;
  else
    return;
  LL now0 = std::chrono::duration_cast<std::chrono::nanoseconds>(
      std::chrono::system_clock::now().time_since_epoch())
...
void SwapGPU::DeploySwapIn(int r_global_index) {
  // check whether the current index, should we swap in variables or remalloc
  // directly
...
void SwapGPU::Append(InfoBlock b) {
...

  {
    ...
    if (async_swap_flag == 1 && global_index > fastiter + 2 * iterlen &&
        global_index < stopswap) {
  ...
  if (async_swap_flag == 1) {
    int r_global_index = (global_index - iter2) % iterlen;
    if ((int)size_sequence.size() > r_global_index &&
        b.size != size_sequence[r_global_index]) {
      async_swap_flag = 0;
  ...
  global_index++;
  if (async_swap_flag == 1 && global_index > fastiter + 2 * iterlen &&
      (global_index - iter2) % iterlen == 0 && global_index < stopswap) {
    deploytime = 0;
  }

  // this code is fake training, just want to get the training time
  // not save intermediate results, if them are used again, just malloc a new
  // one.
  if (faketrain && (past_test_flag == 0 ||
      (global_index < fastiter + faketrain * iterlen))) {
    int delay = delayremove;
    int gindex = b.idx;

    if (b.operation_type == -1)
      blockVis.erase(b.ptr);
    else if (b.size < smallest_block || b.fake) {
    } else
      blockVis[b.ptr] = gindex;

    if (gindex - delay > beginremove && !vecBlock[gindex - delay].fake &&
...
SwapGPU::~SwapGPU() {
  ...
  for (int i = 0; i < (int)vecBlock.size(); i++)
    outfile << vecBlock[i].operation_type << "," << vecBlock[i].ptr << ","
            << vecBlock[i].size << "," << (LL)vecBlock[i].t << ","
            << vecBlock[i].execCnt << "\n";
  outfile.close();
  ...
  for (int i = 0; i < 20; i++) {
    ...
    if ((vecBlock[iter2 + iterlen * (i + 1)].t -
        vecBlock[iter2 + iterlen * i].t) == 0)
      break;
    cout << (LL)(vecBlock[iter2 + iterlen * (i + 1)].t -
        vecBlock[iter2 + iterlen * i].t)
         << endl;
  }
  double t0 = 0, t1 = 1;
  if (iter2 + iterlen * 7 < (int)vecBlock.size())
    t0 = (LL)vecBlock[iter2 + iterlen * 7].t -
        (LL)vecBlock[iter2 + iterlen * 6].t;
  if (iter2 + iterlen * 13 < (int)vecBlock.size())
    t1 = (LL)vecBlock[iter2 + iterlen * 13].t -
        (LL)vecBlock[iter2 + iterlen * 12].t;
  ...
  cout << "ratio:" << (1.0 * (double)maxswapload / (double)maxnoswapload)
       << endl;
...
SwapGPU::SwapGPU(int id, std::shared_ptr<DeviceMemPool> pool)
  ...
  // read data from file for specific operation
  {
    ...
    if(infile.is_open()){
      ...
      while (getline(infile, s)) {
        ...
        if (s.compare(1, pos - 2, "mem_limit_ratio") == 0)
          iss >> mem_limit_ratio;
        else if (s.compare(1, pos - 2, "number_of_swap_blocks") == 0)
          iss >> number_of_swap_blocks;
        else if (s.compare(1, pos - 2, "swap_factor") == 0)
          iss >> swap_factor;
        else if (s.compare(1, pos - 2, "syncfactor") == 0)
          iss >> syncfactor;
        else if (s.compare(1, pos - 2, "swapMemOp") == 0)
          iss >> swapMemOp;
        else if (s.compare(1, pos - 2, "iterlen_threshold") == 0)
          iss >> iterlen_threshold;
        else if (s.compare(1, pos - 2, "DEBUG") == 0)
          iss >> DEBUG;
        else if (s.compare(1, pos - 2, "ignorefactor") == 0)
          iss >> ignorefactor;
        else if (s.compare(1, pos - 2, "justrun") == 0)
          iss >> justrun;
        else if (s.compare(1, pos - 2, "openremovedirect") == 0)
          iss >> openremovedirect;
        else if (s.compare(1, pos - 2, "remove_limit") == 0)
          iss >> overlapcr;
        else if (s.compare(1, pos - 2, "faketrain") == 0)
          iss >> faketrain;
        else if (s.compare(1, pos - 2, "beginremove") == 0)
          iss >> beginremove;
        else if (s.compare(1, pos - 2, "delayremove") == 0)
          iss >> delayremove;
      }
      infile.close();
    }
  ...
  for (int i = 0; i < MAXN; i++)
    for (int j = 0; j < 6; j++) table_sched[j][i].clear();
}
...
void SwapGPU::Setup() {
  ...
  assert(cudnnSetStream(ctx_.cudnn_handle, ctx_.stream) ==
      CUDNN_STATUS_SUCCESS);

  cudaEventCreate(&ctx_.event);
  cudaEventCreate(&ctx_.event2);
  assert(cudnnCreate(&ctx_.cudnn_handle2) == CUDNN_STATUS_SUCCESS);
  assert(cudaStreamCreateWithFlags(&ctx_.stream2, cudaStreamNonBlocking) ==
      cudaSuccess);
  assert(cudnnSetStream(ctx_.cudnn_handle2, ctx_.stream2) ==
      CUDNN_STATUS_SUCCESS);
